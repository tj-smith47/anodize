...
///
/// Idempotency strategy — "check first, act if needed":
///
/// 1. If `branch` names a branch that already exists on `origin`, start the
///    local branch from that remote tip (tracking). Any prior run's content
///    is now the local HEAD, so `git diff --cached` after staging measures
///    what would *actually change* on the remote — not churn generated by
///    committing identical content on top of master.
/// 2. If the staged diff is empty, the remote already has the intended
///    content. Return success without committing or pushing. PR / gh-CLI
///    steps that follow are separately idempotent (they detect
///    "already exists").
/// 3. If the staged diff is non-empty, commit on top of the remote's tip
///    and push as a plain fast-forward. No `--force` or `--force-with-lease`
///    is needed because we rebased onto the remote before staging.
///
/// This removes the prior class of bugs where a retry produced a new local
/// commit with identical *content* but different SHA (timestamp/author),
/// failed the non-fast-forward check, and required a force push to
/// reconcile.
pub(crate) fn commit_and_push_with_opts(
    ...
    if let Some(branch_name) = branch {
        checkout_tracking_remote_if_present(repo_path, branch_name, label)?;
    }
    ...
    // Staged diff is now measured against the actual remote branch tip (if
    // one exists). Empty means the remote already matches desired state.
    ...
    // Plain fast-forward push. Because `checkout_tracking_remote_if_present`
    // positioned the local branch at the remote's tip before we committed,
    // our commit's parent IS the remote's tip — the push is a straight
    // fast-forward regardless of prior run attempts.
...
/// Position a local branch at the remote's current tip if the remote branch
/// exists; otherwise create a fresh branch from the current HEAD.
///
/// This is the machinery that makes `commit_and_push_with_opts` idempotent
/// against prior-run leftovers. After the call:
/// - If `origin/<branch>` exists, local `<branch>` points at that exact
///   commit. `git add` + `git diff --cached` now measures the change the
///   push would make on the remote, so an identical-content retry detects
///   "nothing to do" and the call can exit early.
/// - If `origin/<branch>` does not exist, local `<branch>` is created from
///   the current HEAD (the caller's checkout, typically the fork's
///   default branch). The eventual push creates the remote branch.
fn checkout_tracking_remote_if_present(repo_path: &Path, branch: &str, label: &str) -> Result<()> {
    ...
    if remote_exists {
        // `-B` creates or resets the local branch atomically at the given
        // start point, so repeated runs within one process don't accumulate
        // leftover local branches.
        run_cmd_in(
            repo_path,
            "git",
            &["checkout", "-B", branch, &format!("origin/{}", branch)],
            &format!("{label}: git checkout (track origin/{})", branch),
        )
    } else {
        run_cmd_in(
            repo_path,
            "git",
            &["checkout", "-b", branch],
            &format!("{label}: git checkout -b {} (new branch)", branch),
        )
    }
}
