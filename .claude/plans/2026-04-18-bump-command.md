# `anodize bump` — Spec & Plan

Status: spec, not implemented. Handoff from a cross-repo brainstorm on 2026-04-18.

## Why this, why anodize

cargo-release bumps + commits + tags + publishes as one atomic flow. `cargo set-version` (cargo-edit) bumps only — no git, no inference. `cargo-workspaces` has its own surface. None of them know what anodize knows:

1. **Per-crate automatic level from Conventional Commits.** anodize already parses commits (`stage-changelog::parse_commit_message`) and already reads commits scoped to a crate's paths (`git::get_commits_between_paths`). Deciding `feat→minor`, `fix→patch`, `BREAKING CHANGE→major` *per crate* is a one-line mapping on top of infra that exists today.
2. **"Only bump what changed."** anodize already uses per-crate tag prefixes (`core-v0.4.2`, `cli-v0.4.2`) via `monorepo_tag_prefix()`. It can skip crates with no commits since their last tag. No other tool knows this.
3. **Cascade awareness.** Anodize has the workspace graph and the release-tag history. "Bumping core major means cli must ship too" falls out of that context.
...
5. **Release-policy enforcement.** Anodize already knows protected-branch, signing-required, and `.anodize.yaml` constraints. `bump` refuses states that'd produce an invalid release.

**The headline:** `anodize bump` with no arguments figures out which crates changed, at what level each needs to bump, and shows you. Impossible for any tool that only sees `Cargo.toml`.

## Scope

**In scope (v1):**
- `anodize bump [LEVEL_OR_VERSION] [OPTIONS]`
...
- Bundled changelog: `--commit` also stages the changelog update if anodize's changelog stage can render one
- Composition: `anodize bump --commit && anodize tag` is the intended flow

**Out of scope (v1):**
- Tag creation (`anodize tag`'s job)
- Publishing (`anodize publish`'s job)
- Push (user does this; or `anodize tag --push`)
...
- `--strict` enforcement against `crates[*].version` pins in `.anodize.yaml`
...
- Config hooks: respect existing `TagConfig.major_string_token` / `minor_string_token` / `patch_string_token` (same token set as `anodize tag`)
...
anodize-core       0.4.2     →   0.5.0     minor    3 feat commits
anodize-cli        0.4.2     →   0.4.3     patch    1 fix commit
anodize-stage-foo  0.4.2     →   —         skip     no commits since core-v0.4.2
```
...
This is #9 from the gap list — what separates `anodize bump` from "cargo set-version with a wrapper".
...
- `.anodize.yaml` (self-host) — optional bump config section if we add one
...
Each phase should end with `cargo test -p anodize-cli` green before moving on.
...
5. **Should `bump` ever push?** My vote: no. Pushing is `anodize tag`'s business (and arguably not even that — composable).
6. **How does this interact with `anodize.spec` / `.anodize.yaml` dist config?** At minimum, refuse bumps that would violate pinned versions elsewhere in the spec. Probably v1.1.
7. **Exit codes.** Should match anodize's existing taxonomy (whatever it is). If it doesn't have one, this is a chance to introduce one — see cfgd's `crates/cfgd-core/src/exit.rs` for a recent precedent (Success=0, Error=1, ConfigInvalid=4, etc. with stable wire values locked by test).

## Success criteria for v1

- `anodize bump` with no args in a cfgd-like workspace prints a plan, prompts, and on confirmation edits only the crates that actually have changes since their last tag, each at its inferred level.
- `anodize bump --workspace --dry-run --output json` produces a structured plan a CI script can parse.
- `anodize bump patch -p cli --commit` does the narrowest thing: edits one Cargo.toml, optionally stages changelog, one commit.
- `anodize bump --commit && anodize tag` is the documented release flow and it works end-to-end on the anodize repo itself (dogfood it).
...
```
anodize bump                                  # infer, workspace (if single-crate) or prompt
anodize bump patch -p cli                     # single crate, explicit level
anodize bump 0.5.0 -p core                    # explicit version
anodize bump minor -p core -p cli             # multi-select, same level
anodize bump minor --workspace                # whole workspace, same level
anodize bump --workspace                      # whole workspace, inferred per-crate
anodize bump major -p core --pre rc.1         # prerelease
anodize bump release -p core                  # strip prerelease
anodize bump minor --workspace --exclude scratchpad
anodize bump minor -p core --dry-run --output json
anodize bump patch -p cli --commit --sign
```
