...
- [ ] 2026-04-27 hygiene (H5 — migration): the post-edit hook flags `.unwrap()` / `.expect()` in non-test code as "tracked in known-bugs for migration to ? + Context", but no actual tracking row existed. **Migration scope**: 75 source files outside `tests/`, `*_test.rs`, `*/main.rs`, and `test_helpers.rs` contain at least one `.unwrap()` or `.expect()` line. Many sites are in legitimate `mod tests` blocks within the same file, but the hook is line-based and flags any non-test-marked line. Migration target: convert each `.unwrap()` / `.expect()` outside test code to `?` + `anyhow::Context::context()` so failures surface with a labelled context chain instead of an opaque panic. File list (75): `crates/cli/src/commands/{bump/cargo_edit,bump/plan,healthcheck,helpers,init,jsonschema,publisher,release/{milestones,mod,split},resolve_tag,tag,targets}.rs`, `crates/cli/src/{pipeline,timeout}.rs`, `crates/core/src/{artifact,config,context,defaults_merge,extrafiles,github_client,git,parallel,partial,template,templated_files,util}.rs`, every `crates/stage-*/src/lib.rs`, `crates/stage-announce/src/{bluesky,discord,email,linkedin,mattermost,opencollective,reddit,slack,teams,telegram,twitter}.rs`, `crates/stage-build/src/{binstall,version_sync}.rs`, `crates/stage-publish/src/{aur,aur_source,artifactory,chocolatey,dockerhub,homebrew,krew,nix,scoop,upload,winget}.rs`, `crates/stage-release/src/gitea.rs`. Drain incrementally per-file; each conversion should land with a regression test where the new error path is exercised.
