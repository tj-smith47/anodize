...
The workspace `Cargo.toml` already has `schemars = "0.8"`. Adding `anodize-core` directly lets xtask call `schema_for!(Config)` without going through the CLI crate indirection.
...
git commit -m "chore(xtask): add schemars, serde_json, anodize-core deps for schema-driven docs"
```
...
Run: `cargo run --bin anodize -- jsonschema 2>/dev/null | python3 -c "
import json, sys
...
- Create: `docs/site/content/docs/ci/split-merge-ci.md` (narrative guide for `anodize publish` and `anodize announce` standalone commands, integrated with the Split & Merge workflow)
...
- `anodize release --split` workflow
- `anodize continue --merge` workflow
...
- **`anodize publish`** — when to use it (running publish independently after a release build), what stages it runs (release + publish + blob), typical CI integration (e.g., a manual approval gate between build and publish)
- **`anodize announce`** — when to use it (posting announcements after publish completes), standalone usage, how it reads from dist/
- **`anodize continue --merge`** — how it fits in the split/merge workflow (reference the Split & Merge advanced page for the full workflow)
...
The post-release section lists `anodize jsonschema` CLI command as a TODO, but it already exists and works (`crates/cli/src/commands/jsonschema.rs`). Update the checklist.
